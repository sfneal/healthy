...
# Build the image
echo "Building image: stephenneal/healthy:${TAG}"
docker build -t stephenneal/healthy:"${TAG}" \
...
docker logs -f healthy

while true; do
    if [[ $(docker inspect -f '{{.State.Running}}' healthy) != true ]]; then
        break
    else
        echo "'healthy' container is still running... waiting 3 secs then checking again..."
        sleep 3
    fi
done

# Confirm it exited with code 0
if [[ $(docker inspect -f '{{.State.ExitCode}}' healthy) == 0 ]]; then
    echo "Success: Tests Passed! - stephenneal/healthy:${TAG}"
else
    echo "Error: Tests Failed! - stephenneal/healthy:${TAG}"
    exit 1
fi

# Confirm the image exists
docker image inspect stephenneal/healthy:"${TAG}" > /dev/null 2>&1
